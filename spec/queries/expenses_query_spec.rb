...
RSpec.describe ExpensesQuery do
  ...
  context 'expenses' do
    context 'sorting only' do
      let(:expense1) { create(:expense, created_at: Time.now.to_i.in_milliseconds) }
      let(:expense2) { create(:expense, created_at: 2.minutes.ago.to_i.in_milliseconds) }
      let(:expense3) { create(:expense, created_at: 5.minutes.ago.to_i.in_milliseconds) }

      context 'when empty params' do
        it 'sorts by default sort type (created_at asc)' do
      ...
      context 'when sort params' do
        ...
        it 'sorts by default created_at desc' do
    ...
    context 'filtering only' do
      context 'by category' do
        ...
        context 'when filter second category' do
          let(:params) { { category: categories[1] } }
        ...
        context 'when filter by first category' do
          let(:params) { { category: categories[0] } }
    ...
    context 'sorting & filtering' do
      let(:expense1) { create(:expense, created_at: Time.now.to_i.in_milliseconds, category: categories[0]) }
      let(:expense2) do
        create(:expense, created_at: 2.minutes.ago.to_i.in_milliseconds, category: categories[0])
      end
      let(:expense3) do
        create(:expense, created_at: 5.minutes.ago.to_i.in_milliseconds, category: categories[1])
      end

      context 'filter by category first and sort by created_at desc' do
        let(:params) do
          { sort: 'created_at_desc', category: categories[0] }
        end
      ...
      context 'filter by category first and sort by created_at asc' do
        let(:params) do
          { sort: 'created_at_asc', category: categories[0] }
        end
